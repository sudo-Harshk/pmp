...
5. **Guessing Game:** A 30-second snippet plays **synced** (`roundStartTime` + server clock + `seekTo` drift fix, everyone hears same second). Voting is **concealed**: every screen shows the same `Who submitted this song?` grid with **only other players** (never yourself, no `(You)` label) — submitter's tap is a local dummy, identical `Vote Locked ✅ You guessed {name}` but with zero Firebase write, scoring, or counter impact, so a neighbour can't tell whose song it is. Real votes lock immediately and independently (no global steal). If the video is `101/150/100` unplayable, **Skip Unplayable Track** appears. After **Reveal** (shows `sat out — your track ★ +5` for submitters), a 7-second **Intermission** auto-advances. Double reveal blocked.

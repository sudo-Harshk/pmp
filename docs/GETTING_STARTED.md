...
2. In the lobby, the host sees `🎬 {Room Name}` above the code; everyone sees `🎬 {name} · {code}` in the header. Host picks the play mode (**Guessing Game** / **Casual Jukebox**) and fixes **Songs per player** (stepper 1–10, default 3, lobby-only); guests see the fixed count. The shuffled Fisher–Yates playlist is built once at game start so order is never straight.
3. Host presses **Start Submission**; everyone submits **exactly N** YouTube links (deduped, `?t=`/`&list=` stripped) — the Submit button stays disabled until all N are valid (`Add X more`); extras via devtools are truncated server-side and never stored. Tracks *everyone* submitted are dead rounds (nobody can vote) — flagged `⚠️ skipped at start` and dropped in Guessing mode (Jukebox keeps them); if none remain playable, Start is blocked until different songs are submitted. The host can press **Start Game/Playback** as soon as `tracks.length>0` — not blocked if someone is AFK. Anyone can join with the 4-letter code even after the game started (`SUBMISSION`/`PLAYING`/`REVEAL`/`INTERMISSION`); only `GAMEOVER` blocks (`Game over — ask the host to start a new room`). Leaving players trigger `🔴 {name} left` toasts; joining triggers `🟢 {name} joined`; tab-close is auto-cleaned via `onDisconnect`.
...
5. **Guessing Game:** A 15-second snippet plays **synced** (`roundStartTime` + server clock + `seekTo` drift fix, everyone hears same second). Voting is **concealed**: every screen shows the identical `Who submitted this song?` grid (full roster including yourself, same sorted order) — submitter's tap is a local dummy, identical `Vote Locked ✅` but zero write/score/counter impact, and self-votes are fully void in scoring (no `+10`, never feed the bonus pool). If the video is `101/150/100` unplayable, **everyone** sees `Video unplayable — Anyone can skip` and the same **Skip Unplayable Track** button (anyone can tap during `PLAYING`; `hostNext` PLAYING is open, other transitions remain host-only). After **Reveal** (`sat out — your track ★ +5`), a 5-second **Intermission** auto-advances. Double reveal blocked. Host-leave promotes the first remaining player with `👑 You are now the host` toast.
...
7. After the last track, **GAMEOVER** shows the leaderboard with tiebreak `score → bestRound → name` and confetti. The host sees **🔄 Play Again (same code)** (same room/code/roster, scores zeroed — everyone still inside jumps back to the lobby, leavers rejoin normally) and **End Room** (deletes the room, everyone goes home, code gives `Room not found` after); guests see *Waiting for host to start a new game…*. When the last player leaves, the room auto-deletes — no orphan rooms.
